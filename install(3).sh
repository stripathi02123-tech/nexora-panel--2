...
Environment variables:
  NEXORA_REPO=owner/repo          Default: ${REPO}
...
环境变量：
  NEXORA_REPO=owner/repo          默认：${REPO}
...
setup_runtime_services() {
    ...
    warn "未检测到 systemd 或 OpenRC，跳过 LXC/KVM 服务管理器配置。"
    return 0
}
...
install_service() {
    ...
    if is_systemd; then
        ...
        warn "未检测到 systemd 或 OpenRC；跳过服务注册，直接以后台进程启动 NEXORA。"
        log_file="/var/log/nexora.log"
        pid_file="/run/nexora.pid"
        if command -v nohup >/dev/null 2>&1; then
            nohup /usr/local/bin/nexora server >>"$log_file" 2>&1 &
            echo "$!" > "$pid_file"
            sleep 1
            if kill -0 "$(cat "$pid_file")" >/dev/null 2>&1; then
                log "NEXORA 已作为后台进程启动，PID=$(cat "$pid_file")。"
            else
                warn "NEXORA 后台进程未保持运行，请检查 $log_file。"
            fi
        else
            warn "未找到 nohup，无法自动启动 NEXORA。"
        fi
    fi
}
