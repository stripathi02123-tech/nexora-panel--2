...
service_manager_diagnostics() {
    pid1="$(ps -p 1 -o comm= 2>/dev/null | sed 's/^[[:space:]]*//; s/[[:space:]]*$//' || true)"
    pid1_cmd="$(ps -p 1 -o args= 2>/dev/null | sed 's/^[[:space:]]*//; s/[[:space:]]*$//' || true)"
    init_target="$(readlink -f /sbin/init 2>/dev/null || true)"
    systemd_state="no"
    openrc_state="no"
    is_systemd && systemd_state="yes"
    is_openrc && openrc_state="yes"
    log "服务管理器检测：systemd=${systemd_state} openrc=${openrc_state} PID1=${pid1:-unknown} PID1_CMD=${pid1_cmd:-unknown} /sbin/init=${init_target:-unknown}"
}
...
check_os_compatibility() {
    ...
    if ! is_systemd && ! is_openrc; then
        die "未检测到可用的 systemd 或 OpenRC。请在完整 Linux 主机/完整 VPS 上安装 NEXORA；当前环境可能是 Docker/LXC/受限容器、WSL 或没有 init 的最小环境。NEXORA 需要服务管理器来安装和启动后台服务。"
    fi
    case "$OS_ID" in
