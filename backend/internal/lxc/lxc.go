...
// when nexora started, such as after host boot or service restart.
...
// that autostart slightly after nexora still get prepared before WebSSH opens.
...
func (m *Manager) configureCustomLXCBase(lxcName string, tmpl *Template) error {
	...
	base := []string{
		"# NEXORA custom rootfs base configuration",
...
func (m *Manager) applyResourceLimits(lxcName string, cfg ContainerConfig) error {
	...
	for _, line := range lines {
		trimmed := strings.TrimSpace(line)
		if !strings.Contains(trimmed, "# nexora managed") &&
	...
	newLines = append(newLines, "", "# nexora managed: lxcfs virtualized /proc")
	newLines = append(newLines, "lxc.mount.auto = proc:mixed sys:mixed cgroup:mixed")
	newLines = append(newLines, "", "# nexora managed: mandatory unprivileged container hardening")
	...
	newLines = append(newLines, "", "# nexora managed resource limits (cgroup v2)")
...
const nexoraSeccompProfileDir = "/var/lib/nexora/security/seccomp"
const nexoraCVE202643499SeccompProfile = nexoraSeccompProfileDir + "/lxc-cve-2026-43499.profile"

var cve202643499FutexSeccompRules = []string{
	"# nexora managed: mitigate CVE-2026-43499 from LXC guests by blocking PI futex operations",
...
func ensureCVE202643499SeccompProfile(basePath string) (string, error) {
	...
	if err := os.MkdirAll(nexoraSeccompProfileDir, 0755); err != nil {
		return "", fmt.Errorf("failed to create NEXORA seccomp directory: %v", err)
	}
	hardened := appendMissingSeccompRules(content, cve202643499FutexSeccompRules)
	if err := os.WriteFile(nexoraCVE202643499SeccompProfile, []byte(hardened), 0644); err != nil {
		return "", fmt.Errorf("failed to write NEXORA seccomp profile: %v", err)
	}
	return nexoraCVE202643499SeccompProfile, nil
}
...
func (m *Manager) shiftRootfsForUnprivileged(lxcName string) error {
	...
	rootfsPath := filepath.Join(m.LxcPath, lxcName, "rootfs")
	marker := filepath.Join(rootfsPath, ".nexora-unprivileged-shifted")
...
func (m *Manager) rootfsShifted(lxcName string) bool {
	marker := filepath.Join(m.LxcPath, lxcName, "rootfs", ".nexora-unprivileged-shifted")
...
func (m *Manager) startLXCContainerDaemon(lxcName string) (string, string, []byte, error) {
	logFile := filepath.Join(os.TempDir(), "nexora-"+lxcName+"-start.log")
	consoleLog := filepath.Join(os.TempDir(), "nexora-"+lxcName+"-console.log")
	os.Remove(logFile)
...
if command -v resolvectl >/dev/null 2>&1; then
	resolvectl dns eth0 __NEXORA_LXC_GATEWAY__ 2>/dev/null || true
...
if ! grep -q '^nameserver [1-9]' /etc/resolv.conf 2>/dev/null; then
	echo "nameserver __NEXORA_LXC_GATEWAY__" > /etc/resolv.conf
...
cat >/etc/ssh/sshd_config.d/99-nexora.conf <<'EOF'
PermitRootLogin yes
...
set_sshd_option PubkeyAuthentication __NEXORA_PUBKEY_AUTH__
...
ensure_sshd_runtime_dir
"$SSHD_BIN" -t -f /etc/ssh/sshd_config >/tmp/nexora-sshd-test.log 2>&1 || {
	cat /tmp/nexora-sshd-test.log
	exit 32
}
`
	script = strings.ReplaceAll(script, "__NEXORA_PUBKEY_AUTH__", pubkeyValue)
	script = strings.ReplaceAll(script, "__NEXORA_LXC_GATEWAY__", config.LXCNATNetwork().Gateway)
	if !startService {
		return script
	}
	return script + `
...
func (m *Manager) rootfsCommand(rootfsPath string, args ...string) (*exec.Cmd, error) {
	...
	marker := filepath.Join(cleanRootfsPath, ".nexora-unprivileged-shifted")
	if _, err := os.Stat(marker); err == nil {
...
func safeRootfsCommandArgs(args []string) ([]string, error) {
	...
	if args[0] == "sh" {
		...
		if !isNEXORAManagedRootfsScript(args[2]) {
...
func isNEXORAManagedRootfsScript(script string) bool {
	return strings.Contains(script, "99-nexora.conf") &&
...
// ImportExistingNexoraContainers imports existing LXC containers into the NEXORA
// config. Native NEXORA containers keep ct-{id}; arbitrary LXC names are stored
// in Container.LXCName so Web and CLI can manage the same imported container.
func (m *Manager) ImportExistingNexoraContainers() ([]config.Container, error) {
...
func (m *Manager) replaceRootfsFromTemplate(lxcName string, tmpl *Template) error {
	...
	tmpName := fmt.Sprintf("nexora-reinstall-%s-%s", lxcName, generateRandomString(8))
