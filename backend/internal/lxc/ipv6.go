...
func verifyIPv4SourceUsable(iface, address string) (bool, string) {
	...
	if exec.Command("ip", "-4", "addr", "show", "dev", iface, "to", cidr).Run() != nil {
		output, err := exec.Command("ip", "-4", "addr", "add", cidr, "dev", iface, "label", iface+":nexorascan").CombinedOutput()
		if err != nil {
...
func EnsureAssignedPublicIPv4s(assignments []config.PublicIPv4Assignment) {
	for _, assignment := range assignments {
		...
		if output, err := exec.Command("ip", "-4", "addr", "add", cidr, "dev", iface, "label", iface+":nexora").CombinedOutput(); err != nil {
...
func (m *Manager) applyIPv6Config(lxcName string, ipv6s ...string) error {
	...
	for _, line := range lines {
		...
		if strings.Contains(trimmed, "# nexora managed: public IPv6") ||
	...
	if len(ipv6s) > 0 {
		next = append(next, "", "# nexora managed: public IPv6 routed /128")
...
func installContainerIPv6Init(rootfsPath string, ipv6s ...string) error {
	...
	scriptPath := filepath.Join(rootfsPath, "usr", "local", "sbin", "nexora-ipv6-init")
...
func removeContainerIPv6Init(rootfsPath string) error {
	paths := []string{
		filepath.Join(rootfsPath, "usr", "local", "sbin", "nexora-ipv6-init"),
		filepath.Join(rootfsPath, "etc", "systemd", "system", "nexora-ipv6.service"),
		filepath.Join(rootfsPath, "etc", "systemd", "system", "multi-user.target.wants", "nexora-ipv6.service"),
		filepath.Join(rootfsPath, "etc", "init.d", "nexora-ipv6"),
		filepath.Join(rootfsPath, "etc", "runlevels", "default", "nexora-ipv6"),
	}
	for _, level := range []string{"2", "3", "4", "5"} {
		paths = append(paths, filepath.Join(rootfsPath, "etc", "rc"+level+".d", "S99nexora-ipv6"))
	}
...
func installContainerIPv6Systemd(rootfsPath string) error {
	servicePath := filepath.Join(rootfsPath, "etc", "systemd", "system", "nexora-ipv6.service")
	if err := os.MkdirAll(filepath.Dir(servicePath), 0755); err != nil {
		return err
	}
	service := `[Unit]
Description=NEXORA IPv6 setup
After=network-online.target network.target
Wants=network-online.target

[Service]
Type=oneshot
ExecStart=/usr/local/sbin/nexora-ipv6-init
RemainAfterExit=yes
...
`
	...
	return replaceSymlink("../nexora-ipv6.service", filepath.Join(wantsDir, "nexora-ipv6.service"))
}

func installContainerIPv6OpenRC(rootfsPath string) error {
	initPath := filepath.Join(rootfsPath, "etc", "init.d", "nexora-ipv6")
	if err := os.MkdirAll(filepath.Dir(initPath), 0755); err != nil {
		return err
	}
	initScript := `#!/sbin/openrc-run
name="NEXORA IPv6 setup"
description="Apply NEXORA IPv6 settings"
...
start() {
	ebegin "Applying NEXORA IPv6"
	/usr/local/sbin/nexora-ipv6-init
	eend $?
}
`
	...
	return replaceSymlink(filepath.Join("..", "..", "init.d", "nexora-ipv6"), filepath.Join(runlevelDir, "nexora-ipv6"))
}
...
func installContainerIPv6SysV(rootfsPath string) error {
	...
	initPath := filepath.Join(initDir, "nexora-ipv6")
	initScript := `#!/bin/sh
### BEGIN INIT INFO
# Provides:          nexora-ipv6
# Required-Start:    $network
# Required-Stop:
# Default-Start:     2 3 4 5
# Default-Stop:
# Short-Description: NEXORA IPv6 setup
### END INIT INFO
...
`
	...
	for _, level := range []string{"2", "3", "4", "5"} {
		...
		if err := replaceSymlink(filepath.Join("..", "init.d", "nexora-ipv6"), filepath.Join(rcDir, "S99nexora-ipv6")); err != nil {
			return err
		}
	}
	return nil
