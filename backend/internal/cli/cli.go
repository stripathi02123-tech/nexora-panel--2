...
import (
	...
	"nexora/internal/lxc"
	"nexora/internal/version"
)

var manager = lxc.NewManager()

const (
	nexoraBackupDir             = "/root/nexora-backups"
	nexoraNewBinaryPath         = "/usr/local/bin/nexora.new"
	libvirtDefaultNetworkMarker = "/var/lib/nexora/kvm/default-network.created"
)
...
var cliTranslations = map[string]string{
	...
	"再见":                 "Goodbye",
	"无效选择":               "Invalid choice",
	"NEXORA - LXC 容器管理器": "NEXORA - Container Manager",
	"Web 面板":             "Web panel",
	"端口":                 "port",
	"运行中":                "running",
	"已停止":                "stopped",
	...
	"检查并升级 NEXORA":       "Check and upgrade NEXORA",
	"卸载 NEXORA":          "Uninstall NEXORA",
	...
	"升级只会替换 /usr/local/bin/nexora，并保留 /root/.nexora 里的配置、容器数据和任务记录。": "The upgrade only replaces /usr/local/bin/nexora and keeps configuration, container data, and task records under /root/.nexora.",
	"升级需要 root 权限。请使用: sudo nexora cli":                              "Upgrade requires root privileges. Use: sudo nexora cli",
	...
	"升级包内未找到 nexora 二进制":                    "No nexora binary found in the upgrade package",
	"将 /var/lib/lxc 里的容器导入 NEXORA 配置。":      "Import containers under /var/lib/lxc into NEXORA configuration.",
	...
	"已导入":             "Imported",
	"个容器":             "containers",
	"将删除 NEXORA 服务和 /usr/local/bin/nexora。": "This will remove the NEXORA service and /usr/local/bin/nexora.",
	"同时会删除 /root/.nexora、/var/lib/lxc、/var/lib/nexora、镜像缓存、备份、临时文件、/swapfile 和 NEXORA 网络规则。": "It will also remove /root/.nexora, /var/lib/lxc, /var/lib/nexora, image caches, backups, temporary files, /swapfile, and NEXORA network rules.",
	"卸载需要 root 权限。":                 "Uninstall requires root privileges.",
	"请运行: sudo nexora cli --no-web": "Run: sudo nexora cli --no-web",
	"输入 uninstall 继续卸载":             "Type uninstall to continue uninstalling",
	"NEXORA 已卸载。":                   "NEXORA has been uninstalled.",
	"服务、二进制、配置、容器/虚拟机、本地镜像、缓存、备份、临时文件和 NEXORA 网络规则均已删除。":         "Service, binary, configuration, containers/VMs, local images, cache, backups, temporary files, and NEXORA network rules have been removed.",
	"检测到非 NEXORA 虚拟机仍在使用 libvirt default 网络，已保留 default/virbr0。": "Non-NEXORA VMs are still using the libvirt default network, so default/virbr0 has been kept.",
...
func printMenu() {
	...
	cliPrintln("  11. 检查并升级 NEXORA")
	cliPrintln("  12. 卸载 NEXORA")
...
func cliToggleWebPanel() {
	if isWebPanelRunning() {
		if err := stopService("nexora"); err != nil {
	...
	if err := startService("nexora"); err != nil {
...
func cliUpgradeSystem(reader *bufio.Reader) {
	cliPrintln("\n--- 检查并升级 NEXORA ---")
	cliPrintln("升级只会替换 /usr/local/bin/nexora，并保留 /root/.nexora 里的配置、容器数据和任务记录。")

	if os.Geteuid() != 0 {
		cliPrintln("升级需要 root 权限。请使用: sudo nexora cli")
		return
	}

	repo := strings.TrimSpace(os.Getenv("NEXORA_REPO"))
	if repo == "" {
		repo = version.Repo
	}
	if repo == "" {
		cliPrintln("自动升级需要配置 NEXORA_REPO（例如 stripathi02123-tech/nexora-panel--2）。")
		return
	}
...
func fetchLatestReleaseFallback(repo, assetName string) (*githubRelease, error) {
	...
	req.Header.Set("User-Agent", "nexora-updater/"+version.Current())
...
func setGitHubRequestHeaders(req *http.Request) {
	req.Header.Set("User-Agent", "nexora-updater/"+version.Current())
	token := strings.TrimSpace(os.Getenv("NEXORA_GITHUB_TOKEN"))
	if token == "" {
...
func releaseArchiveAssetName(goarch string) (string, error) {
	switch goarch {
	case "amd64", "arm64":
		return fmt.Sprintf("nexora-linux-%s.tar.gz", goarch), nil
	default:
...
func upgradeFromReleaseAsset(assetURL, latest, assetName string) error {
	tmpDir, err := os.MkdirTemp("", "nexora-upgrade-*")
	if err != nil {
		return err
	}
	...
	newBinary, err := findFile(tmpDir, "nexora")
	if err != nil {
		return err
	}

	backupDir := nexoraBackupDir
	if err := os.MkdirAll(backupDir, 0700); err != nil {
		return err
	}
	backupName := fmt.Sprintf("nexora.%s.%s", safeReleaseBackupComponent(latest), time.Now().Format("20060102-150405"))
	if _, err := os.Stat("/usr/local/bin/nexora"); err == nil {
		backupPath, err := copyFileToBackup("/usr/local/bin/nexora", backupName, 0755)
		if err != nil {
	...
	tmpBin := nexoraNewBinaryPath
	if err := copyFileToUpgradeTemp(newBinary, 0755); err != nil {
		return err
	}
	if err := os.Rename(tmpBin, "/usr/local/bin/nexora"); err != nil {
		return err
	}
	if err := os.Chmod("/usr/local/bin/nexora", 0755); err != nil {
		return err
	}
...
func findFile(root, name string) (string, error) {
	...
	if found == "" {
		return "", fmt.Errorf("升级包内未找到 nexora 二进制")
	}
	return found, nil
}
...
func copyFileToBackup(src, fileName string, mode os.FileMode) (string, error) {
	...
	dst := filepath.Join(nexoraBackupDir, fileName)
...
func copyFileToUpgradeTemp(src string, mode os.FileMode) error {
	out, err := os.OpenFile(nexoraNewBinaryPath, os.O_CREATE|os.O_TRUNC|os.O_WRONLY, mode)
	if err != nil {
		return err
	}
...
func isWebPanelRunning() bool {
	...
	if commandExists("rc-service") {
		cmd := exec.Command("rc-service", "nexora", "status")
		return cmd.Run() == nil
	}
	return false
}
...
func cliImportExistingContainers() {
	cliPrintln("\n--- 导入现有 LXC 容器 ---")
	cliPrintln("将 /var/lib/lxc 里的容器导入 NEXORA 配置。")
	cliPrintln("导入后会保留真实 LXC 名称，Web 和 CLI 都能管理同一个容器。")

	imported, err := manager.ImportExistingNexoraContainers()
	if err != nil {
		cliPrintf("导入失败: %v\n", err)
		return
	}
...
func cliUninstall(reader *bufio.Reader) {
	cliPrintln("\n--- 卸载 NEXORA ---")
	cliPrintln("将删除 NEXORA 服务和 /usr/local/bin/nexora。")
	cliPrintln("同时会删除 /root/.nexora、/var/lib/lxc、/var/lib/nexora、镜像缓存、备份、临时文件、/swapfile 和 NEXORA 网络规则。")

	if os.Geteuid() != 0 {
		cliPrintln("卸载需要 root 权限。")
		cliPrintln("请运行: sudo nexora cli --no-web")
		return
	}
	...
	removeNEXORALibvirtDefaultNetwork()
	cleanupNEXORANetworking()
	removeNEXORAHostHooks()
	removeNEXORAQuotaRecords()
	stopAndRemoveService()
	removePath("/usr/local/bin/nexora")
	removePath("/etc/sysctl.d/99-nexora.conf")
	removePath("/var/log/nexora.log")
	removePath("/var/log/nexora.err")
	removePath("/root/.nexora")
	removePath("/var/lib/lxc")
	removePath("/var/lib/nexora")
	removePath("/var/cache/lxc")
	removePath("/var/cache/nexora")
	removePath("/root/nexora-backups")
	removeNEXORATmpFiles()
	removeNEXORASwapfile()

	reloadSysctl()

	fmt.Println()
	cliPrintln("NEXORA 已卸载。")
	cliPrintln("服务、二进制、配置、容器/虚拟机、本地镜像、缓存、备份、临时文件和 NEXORA 网络规则均已删除。")
}
...
func destroyAllKVMDomains() {
	...
	for _, line := range strings.Split(string(out), "\n") {
		...
		if isNEXORAKVMDomain(name) {
			removeKVMDomain(name)
		}
	}
}

func isNEXORAKVMDomain(name string) bool {
	...
	if dirExists("/var/lib/nexora/kvm/instances/" + name) {
		return true
	}
	out, err := exec.Command("virsh", "dumpxml", name).Output()
	return err == nil && strings.Contains(string(out), "/var/lib/nexora/kvm/")
}
...
func removeNEXORALibvirtDefaultNetwork() {
	if !commandExists("virsh") || !fileExists(libvirtDefaultNetworkMarker) {
		return
	}
	if libvirtDefaultUsedByNonNEXORADomain() {
		cliPrintln("检测到非 NEXORA 虚拟机仍在使用 libvirt default 网络，已保留 default/virbr0。")
		return
	}
	fmt.Println("Removing NEXORA-created libvirt default network...")
	runQuiet("virsh", "net-destroy", "default")
	runQuiet("virsh", "net-undefine", "default")
	removePath(libvirtDefaultNetworkMarker)
}

func libvirtDefaultUsedByNonNEXORADomain() bool {
	if !commandExists("virsh") {
		return false
	}
	out, err := exec.Command("virsh", "list", "--all", "--name").Output()
	...
	for _, line := range strings.Split(string(out), "\n") {
		name := strings.TrimSpace(line)
		if name == "" || isNEXORAKVMDomain(name) {
			continue
		}
...
func cleanupNEXORANetworking() {
	removeNEXORANATRules()
	cleanupNEXORAIPv6Runtime()
	cleanupNEXORAIPv6BridgeRoutes()
...
func cleanupNEXORAIPv6Runtime() {
	if config.AppConfig == nil {
		return
	}
	for _, c := range config.AppConfig.Containers {
		cleanupNEXORAContainerIPv6(c)
	}
}

func cleanupNEXORAContainerIPv6(c config.Container) {
	bridge := "lxcbr0"
	if c.IsKVM() {
		bridge = "virbr0"
	}
...
func cleanupNEXORAIPv6BridgeRoutes() {
	if !commandExists("ip") {
		return
	}
	for _, bridge := range []string{"lxcbr0", "virbr0"} {
...
func removeNEXORANATRules() {
	if commandExists("iptables") {
		for {
			out, err := exec.Command("sh", "-c", "iptables -t nat -L PREROUTING -n --line-numbers 2>/dev/null | grep 'nexora-' | awk '{print $1}' | head -n 1").Output()
...
func removeNEXORAHostHooks() {
	runQuiet("systemctl", "stop", "nexora-kvm-ipv6.service")
	runQuiet("systemctl", "disable", "nexora-kvm-ipv6.service")
	runQuiet("rc-service", "nexora-kvm-ipv6", "stop")
	runQuiet("rc-update", "del", "nexora-kvm-ipv6", "default")
	removePath("/usr/local/sbin/nexora-kvm-ipv6-init")
	removePath("/etc/systemd/system/nexora-kvm-ipv6.service")
	removePath("/etc/local.d/nexora-kvm-ipv6.start")
	removePath("/etc/network/if-up.d/nexora-kvm-ipv6")
}

func removeNEXORAQuotaRecords() {
	for _, path := range []string{"/etc/projects", "/etc/projid"} {
		...
		for _, line := range strings.Split(string(data), "\n") {
			if strings.TrimSpace(line) == "" || strings.Contains(line, "nexora-") {
				continue
			}
...
func removeNEXORATmpFiles() {
	for _, pattern := range []string{"/tmp/nexora-*", "/tmp/nexora.*"} {
...
func stopAndRemoveService() {
	if commandExists("systemctl") {
		runQuiet("systemctl", "stop", "nexora")
		runQuiet("systemctl", "disable", "nexora")
		removePath("/etc/systemd/system/nexora.service")
		runQuiet("systemctl", "daemon-reload")
		runQuiet("systemctl", "reset-failed", "nexora")
	}

	if commandExists("rc-service") {
		runQuiet("rc-service", "nexora", "stop")
	}
	if commandExists("rc-update") {
		runQuiet("rc-update", "del", "nexora", "default")
	}
	removePath("/etc/init.d/nexora")
}
